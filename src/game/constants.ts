...
/**
 * 果物を追加した直後、接触継続（`collisionActive`）も合体判定の入力に含める物理ステップ数（R-05）。
 *
 * 合体で生まれた果物は既存の同 tier 果物と**重なった状態で出現する**ことがあり、
 * その組み合わせでは接触開始（`collisionStart`）が期待どおり届かない場合がある。
 * 生成直後の数ステップだけ接触継続も拾うことで、合体漏れ（見た目はくっついているのに
 * 合体しない）を防ぐ。常時拾うと静止した山全体が毎ステップ判定対象になり NFR-01 の
 * 予算を圧迫するため、窓は短く保つ。
 */
export const SPAWN_ACTIVE_CONTACT_STEPS = 3;

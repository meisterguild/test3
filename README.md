...
| パラメータ | 例 | 内容 |
| ---------------- | ------------------ | ---------------------------------------------------------------------------------- |
| `autodrop` | `?autodrop=0` | 暫定の自動ドロップを止める（入力実装までの足場。既定は有効） |
| `stress` | `?stress=60` | 指定個数の果物を連続投入する（フレームレート計測用） |
...
# ブラウザで http://localhost:4173/?stress=60&autodrop=0&fps=1 を開き、
# 「果物 60」になった後の fps 表示を 10 秒ほど観察する
```
